...
import 'package:app_amoc_ararangua/features/domain/usecases/create_account_with_email_and_password_use_case.dart';
...
void main() {
  ...
  setUp(() {
    ...
    usecase = CreateAccountWithEmailAndPasswordUseCase(repository);
  });

  group('CreateAccountWithEmailAndPasswordUseCase success', () {
    test('should return an AccountEntity', () async {
      ...
      when(() => repository.createAccountWithEmailAndPassword(email, password))
          .thenAnswer((invocation) async => const Right(tAccount));
      // Act
      final result = await usecase(email, password);
      // Assert
      expect(result, const Right(tAccount));
      verify(() => repository.createAccountWithEmailAndPassword(email, password)).called(1);
    });
  });

  group('CreateAccountWithEmailAndPasswordUseCase failure', () {
